...
function KnowledgeItem({item}) {
    ...
    const handleBrainClick = () => {
        if (itemBrained) {
            KnowledgeApi.RemoveBrainFromKnowledgeItem(item.topicId, item.id, currentUser.id).then(success => {
                if (success) {
            ...
            KnowledgeApi.AddBrainToKnowledgeItem(item.topicId, item.id, currentUser.id).then(success => {
                if (success) {
